...
## Testing notes
- Backend: 138 tests (incl. reminder tests: reconcile idempotency, quiet-hours deferral, cancel-skips, dispatch idempotency/failure-retry; and WhatsApp template tests: per-kind `build_template` params, 24h button payloads, `type: template` Graph payload shape, `type: "button"` inbound tap parse, text fallback when no template/creds). Live-Anthropic conversation tests are `skipUnless(ANTHROPIC_API_KEY)` — including `test_full_reschedule_flow_moves_appointment` / `test_full_cancel_flow_cancels_appointment`, which assert the DB actually changed (guarding against the model faking a confirmation without calling the tool). `ANTHROPIC_API_KEY` is set in `.env` (model `claude-haiku-4-5` — cheap), so these run. Live conversation flow is verified working via the dev chat sandbox and real WhatsApp.
...
- Next (per revised plan): finish Phase 2 hardening → Phase 3 → Phase 4 (superuser-operated multi-tenant admin: superuser adds/deletes clinics, manages subscriptions + pay status manually, payment handled off-platform — no self-onboarding, no payment processor). **Production deployment is deferred until after Phase 4**, followed by a complete professional UI revamp. CI runs build+test from now, but prod deploy/infra is not stood up per-phase. Authoritative plan: `~/Downloads/ai-receptionist-build-plan.md`.
  - **Remaining template gap:** the owner digest has no template (its body is dynamically composed), so it still uses the plain-text fallback and only delivers inside the 24h window. Restructure `digest.py` into a fixed template shape before relying on it in production. Meta delivered/read status reconcile + template-failure surfacing on the dashboard are also still to build.
